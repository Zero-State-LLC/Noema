...
**Freeze.** `docs/HOSTED-ALPHA-FREEZE.md` · pin `ecab55e` · Worker `a77af067-8a36-40b0-ac04-f213e1063379`  
...
Frozen surfaces are locked by `spec-compat.json` `frozen_release` and `workers/noema/test/hosted-alpha-freeze.test.ts`. Later work that changes admission, seal, Genesis, verbs, or room bound must `UNFREEZE` in the same PR. Freeze name `hosted-alpha-0.12.1` pins deployed runtime `ecab55e` / Worker `a77af067`.
