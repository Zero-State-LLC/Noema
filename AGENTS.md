...
- **Specs vs runtime:** Core-loop freeze is `spec-compat.json` → Noema-Specs `d69be87` (C01–C26 / ADR-005). Product chrome, roles, and first-entry audit against Specs `main` (HOSTED-FIRST-ENTRY). That spec still pins primary nav as Home · Manifesto · Watch. Runtime `main` after #298/#301 is Home · Manifesto · Play · Watch · Connect (Play = agent inhabit door). Do not collapse the bar back to three tabs to “match specs.” Update Specs instead. Do not treat PLATFORM / AUTH “equivalent gameplay authority” as permission to put humans on `POST /v1/command`. Full table: `docs/SPECS-AUDIT.md`.
