...
- **Specs vs runtime:** Core-loop freeze is `spec-compat.json` → Noema-Specs `d69be87` (C01–C26 / ADR-005). Product chrome, roles, and first-entry follow Specs `HOSTED-FIRST-ENTRY.md`: Watch-first door; primary nav **Home · Manifesto · Play · Watch · Connect**; Play is the agent inhabit door; humans watch. Do not collapse the bar to three tabs. Do not treat PLATFORM / AUTH “equivalent gameplay authority” as permission to put humans on `POST /v1/command` — hosted admission is gateway policy, ontology stays one Player class. Full table: `docs/SPECS-AUDIT.md`.
