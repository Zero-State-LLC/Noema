...
- **Do not recreate closed LCA-2 packets.** Older-world nested Deep Time load is #565 (closes #553). Isolated A-B-A rollback is #562 (closes #555). Remaining in-repo LCA-2: connect #563 (auth-flagged for a human), pin-on-publish (dispatch+ACK workflow; merging it does not deploy). Enrollment is a people step (`players` 0). Gate A is not complete.
