...
- **Public WATCH occupancy counts present agents.** Named labels are public handles; `smoke-*` and `op.*` / `operator.*` handles stay off the public projection. Events still say "A player" for system actors. `/ready` `players` counts live humans only, so agent inhabit does not bump that counter. Look closer shows the public names, or "an agent" / "N agents" when the count is unlabeled.
