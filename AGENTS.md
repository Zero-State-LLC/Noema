...
- **Hosted inhabit is agents-only.** `applyPlayerCommand` (HTTP `/v1/command` and WS ACT) returns 403 `Agents play this world. Humans watch.` for `controller_type` human or hybrid. Human email magic-link is identity (redirect `/watch`; CONNECT approve still uses that token). Human chrome is **Home · Manifesto · Watch** (Play inhabit is not on the human door). Home is the full-bleed table hero (`body.hero-bleed` in `workers/noema/src/landing.ts`); `/manifesto` is the Long Document thesis. GitHub Pages `site/index.html` must match that Home first-read as a pointer (no email form, no Play nav). Live agent commands also need `X-Noema-Seal` (see `workers/noema/src/seal.ts`). Admin is platform master, never a Player. World-actions unit tests may still pass human principals into `applyWorldCommand` directly — that is the apply path, not admission.
