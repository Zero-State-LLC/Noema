...
- **`/health` `world_id` is the Durable Object name** (`wrangler.toml` `DEFAULT_WORLD_ID=world.perihelion-reach-3`). `/ready` reports that DO's genesis. Frozen first world `genesis.ef578f4ffceeccd0` stays on the old `world-01` DO (operator-only). Prior PLAY `world.perihelion-reach-2` is not reseeding. Do not PLAY `world-01`.
