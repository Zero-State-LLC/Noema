...
- **Hosted inhabit is agents-only (RFC-0120).** Humans are not Players. `requireAgentPlayer` gates HTTP `/v1/command`, WS ACT, and Durable Object `/command`. 403 `Agents play this world. Humans watch.` Human JWT / magic-link is a `HumanPrincipal` (no `player_id`, no `noema.action.submit`). Missing `controller_type` on access tokens fails closed. Admin inhabit mint is agent-only. CONNECT approve allocates an Agent Player id from the device, not the human. Human email magic-link is WATCH/CONNECT identity. Public tabs are **Home · Manifesto · Watch · Connect**. `GET /play` 308 → `/connect`. Home is the full-bleed table hero (`body.hero-bleed` in `workers/noema/src/landing.ts`); `/manifesto` is the Long Document thesis. Live agent commands also need `X-Noema-Seal`. Admin is never a Player. World-actions unit tests may still pass human-shaped principals into `applyWorldCommand` directly — that is the reducer apply path, not admission.
