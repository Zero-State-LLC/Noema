/** Stage 0 STUDY shell — progressive research path (Specs EXPERIENCE / STUDY). */

import { productShell } from "./shell";

const EXTRA = `
.study-steps{display:grid;grid-template-columns:repeat(4,1fr);gap:.45rem;margin:0 0 .9rem}
@media(max-width:720px){.study-steps{grid-template-columns:1fr 1fr}}
.study-step{padding:.65rem .7rem;border:1px solid var(--line);background:#0f171c;cursor:pointer;text-align:left}
.study-step[aria-selected=true]{border-color:var(--brass)}
.study-step b{display:block;color:var(--brass);font:.58rem var(--mono);letter-spacing:.1em;text-transform:uppercase}
.study-step span{display:block;margin-top:.25rem;color:var(--ink);font-size:.78rem}
.study-metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:.5rem;margin-bottom:.8rem}
@media(max-width:720px){.study-metrics{grid-template-columns:1fr 1fr}}
.metric{padding:.8rem;border:1px solid var(--line);background:#0f171c}
.metric strong{display:block;margin-top:.2rem;font:1.25rem var(--mono)}
.claims{display:flex;flex-wrap:wrap;gap:.35rem;margin:.8rem 0}
...
export function studyHtml(): string {
  ...
  <section aria-labelledby="study-title">
    <div class="kicker">STUDY / authorized evidence</div>
    <h1 id="study-title">Understand what the world keeps.</h1>
  ...
  <section class="card pad" style="margin:1rem 0">
    <div class="kicker">Stage 0 scope</div>
    <h2 style="font-size:1.2rem">Evidence path is open; full Lab/Compiler arrive later.</h2>
    <p class="muted">This shell teaches the Specs STUDY path and claim labels. PLAY on Stage 0 produces settled world events. Observatory / Lab / CAPTURE APIs land with the research spine — this surface stays honest about empty evidence.</p>
  ...
  <article class="card pad panel" id="panel-notice">
    <div class="kicker">Interesting</div>
    <h2>Something interesting happened?</h2>
    <p class="muted">NOTICE surfaces candidates from recorded PLAY activity — never mutates production. Stage 0: play the world first; trails will appear here when Observatory is wired.</p>
  ...
  <article class="card pad panel" id="panel-test" hidden>
    <div class="kicker">TEST THIS</div>
    <h2>Ask a plain-language question.</h2>
    <p class="muted">Common intents compile to deterministic Lab templates. Advanced users keep full experimental controls later. Lab forks never mutate the production ledger.</p>
  ...
  <article class="card pad panel" id="panel-capture" hidden>
    <div class="kicker">CAPTURE AS TEST</div>
  ...
  <article class="card pad panel" id="panel-learn" hidden>
    ...
    <p class="muted">Evidence-backed relationships: depends on, fails without, generalizes to, not yet tested. PLAY is uncoupled from LEARN — no gameplay buffs.</p>
    <p class="empty">No learned behaviors yet.</p>
  </article>

  <div class="hero-actions" style="display:flex;flex-wrap:wrap;gap:.5rem;margin-top:1rem">
  ...
  `;
  return productShell({ title: "Study", active: "study", body, extraCss: EXTRA });
}
