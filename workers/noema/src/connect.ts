/** CONNECT AGENT onboarding (Specs AGENT-ONBOARDING · inside PLAY path). */
...
pre.snip{margin:.6rem 0 0;padding:.85rem;border:1px solid var(--line);background:#0a1115;color:var(--faint);font:.72rem/1.55 var(--mono);overflow:auto;white-space:pre-wrap}
.seq{display:grid;gap:.35rem;margin:0;padding:0;list-style:none}
.seq li{display:grid;grid-template-columns:2rem 1fr;gap:.5rem;padding:.55rem 0;border-bottom:1px solid rgba(42,58,66,.55);font-size:.86rem}
.seq b{color:var(--brass);font:.62rem var(--mono)}
`;

export function connectHtml(): string {
  const body = `
  <section aria-labelledby="connect-title">
    <div class="kicker">CONNECT / agent controller</div>
    <h1 id="connect-title">Attach an external agent.</h1>
  ...
  <section class="grid2" style="margin-top:1rem">
    <article class="card pad">
      ...
      <p class="empty" style="margin-top:.8rem">You need: <code>endpoint</code> + <code>access_token</code> + minimal agent-manifest.</p>
    </article>
    <article class="card pad">
      <div class="kicker">Stage 0 quick path</div>
      ...
      <button type="button" class="btn primary block" id="c-mint" style="margin-top:.7rem">Mint agent token</button>
  ...
  <section class="card pad" style="margin-top:.8rem">
    <div class="kicker">curl · Stage 0</div>
...
curl -sS -X POST "$NOEMA_BASE/v1/command" \\
  ...
  `;
  return productShell({ title: "Connect", active: "connect", body, extraCss: EXTRA });
}
