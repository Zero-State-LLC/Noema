...
describe("GC2-S13 mapper", () => {
  it("keeps relay as MULTI_CYCLE_CLASS, adds workshop, and stays silent", () => {
    ...
    expect(isMultiCycleClass("generator")).toBe(false);
...
describe("GC2-S13 world path", () => {
  it("constructs an IN_PROGRESS workshop, then the same entity_id goes live after one WAIT", async () => {
    ...
    expect(storageBeforeGen - w.players[p.player_id].budgets.storage).toBe(CONSTRUCT_COSTS.generator.storage);
    expect(isInProgress(w.rooms["room.yard"].entities.find((e) => e.infra_type === "generator")!)).toBe(false);
