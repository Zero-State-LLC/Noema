...
function world(): WorldRuntime {
  return {
    world_id: "test.hosted-canonical.gc5-s8",
    world_name: "Test",
    cycle: 0,
    sequence: 0,
...
describe("GC5-S8 mapper", () => {
  it("parses market notes and keeps help and TRADE quiet", () => {
    const parsed = parseHumanCommand('market "Energy for storage."');
    expect(parsed.ok).toBe(true);
    if (parsed.ok) {
      expect(parsed.action).toEqual({
        verb: "MESSAGE",
        arguments: { surface: "TRADE_NOTICE", text: "Energy for storage." },
      });
    }
    const trade = parseHumanCommand("trade player.vesper offer=energy:3 want=storage:1");
    expect(trade.ok).toBe(true);
    if (trade.ok) expect(trade.action.verb).toBe("TRADE");
...
describe("GC5-S8 world path", () => {
  it("posts a public trade notice, keeps last 1, and rejects hidden rooms", async () => {
    const w = world();
    ...
    const blocked = await run(hidden, q, "MESSAGE", { surface: "TRADE_NOTICE", text: "Secret." });
    expect(blocked.ok).toBe(false);
    expect(blocked.error?.code).toBe("NOT_OBSERVABLE");
    expect(hidden.rooms["room.vault"].trade_notice).toBeUndefined();
  });
});
