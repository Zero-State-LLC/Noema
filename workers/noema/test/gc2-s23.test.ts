...
function world(): WorldRuntime {
  return {
    world_id: "test.hosted-canonical.gc2-s23",
    world_name: "Test",
    cycle: 0,
    sequence: 0,
...
describe("GC2-S23 mapper", () => {
  it("keeps SHARE cost, caps at five co-owners, and stays silent", () => {
...
describe("GC2-S23 world path", () => {
  it("lets the owner name a fifth co-owner and rejects a sixth", async () => {
    const w = world();
    ...
    const g = principal("player.grebe");
    ...
    w.players[f.player_id].handle = "Auk";
    w.players[f.player_id].budgets = cloneBudgets(DEFAULT_BUDGETS);
    const shop = w.rooms["room.hub"].entities.find((ent) => ent.infra_type === "workshop")!;
    const entityId = shop.entity_id;

    for (const partner of [b, c, d, e]) {
      w.players[a.player_id].budgets = cloneBudgets(DEFAULT_BUDGETS);
      expect(
        (await run(w, a, "BUILD", { operation: "SHARE", entity_id: entityId, player_id: partner.player_id })).ok,
      ).toBe(true);
    }

    const byCoOwner = await run(w, e, "BUILD", {
      operation: "SHARE",
      entity_id: entityId,
    ...
    w.players[a.player_id].budgets = cloneBudgets(DEFAULT_BUDGETS);
    const fifth = await run(w, a, "BUILD", {
      operation: "SHARE",
      entity_id: entityId,
      player_id: f.player_id,
    });
    expect(fifth.ok).toBe(true);
    expect(fifth.events?.map((ev) => ev.event_type).sort()).toEqual(["BUDGET_CONSUMED", "ENTITY_UPDATE"]);
    expect(JSON.stringify(fifth.events || [])).not.toMatch(/STRUCTURE_/);
    ...
    expect(after.co_owner_5_id).toBe(f.player_id);
    expect(fifth.observation?.consequence).toMatch(/share the workshop with Auk/i);

    const sixth = await run(w, a, "BUILD", {
      operation: "SHARE",
      entity_id: entityId,
