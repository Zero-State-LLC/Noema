...
function world(): WorldRuntime {
  return {
    world_id: "test.hosted-canonical.gc2-s9",
    world_name: "Test",
    cycle: 0,
    sequence: 0,
...
describe("GC2-S9 world path", () => {
  it("constructs an IN_PROGRESS relay, then the same entity_id goes live after one WAIT", async () => {
    const w = world();
    ...
    expect(created.infra_type).toBe("relay");
    expect(isInProgress(created)).toBe(true);
    expect(isRelayEntity(created)).toBe(false);
    expect(collectLiveRelays(w.rooms)).toHaveLength(0);

    const again = await run(w, p, "BUILD", { operation: "CONSTRUCT", class: "relay" });
    ...
    expect(isRelayEntity(same)).toBe(true);
    expect(collectLiveRelays(w.rooms).map((e) => e.entity_id)).toEqual([created.entity_id]);
  ...
  it("salvages an in-progress relay with no scar and no live leftover", async () => {
    const w = world();
    const p = principal("player.nacre");
    await run(w, p, "ENTER_WORLD");
    w.players[p.player_id].budgets = cloneBudgets(DEFAULT_BUDGETS);
    ...
    const hidden = await run(w, p, "BUILD", { operation: "CONSTRUCT", class: "relay" });
